...
if __name__ == '__main__':
    ...
    nth = 8
    df = pd.DataFrame(data={data_name: fnames, **labels}, dtype='uint8')
    f1 = df.index % nth == 0
    f2 = df.index % nth == 1

    df.to_csv(os.path.join(export_dir, 'data.csv'), index=False)
    df[f1].to_csv(os.path.join(export_dir, 'test.csv'), index=False)
    df[f1 | f2].to_csv(os.path.join(export_dir, 'val.csv'), index=False)
    df[~(f1 | f2)].to_csv(os.path.join(export_dir, 'train.csv'), index=False)
    ...
    df.to_csv(os.path.join(export_dir, 'data_n.csv'), index=False)
    df[f1].to_csv(os.path.join(export_dir, 'test_n.csv'), index=False)
    df[f1 | f2].to_csv(os.path.join(export_dir, 'val_n.csv'), index=False)
    df[~(f1 | f2)].to_csv(os.path.join(export_dir, 'train_n.csv'), index=False)
