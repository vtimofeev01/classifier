...
class Backbone_nFC(nn.Module):
    def __init__(self, class_num, model_name='resnet50_nfc'):
        ...
        print(f'[Model] Backbone name: {self.backbone_name}')
        print(f'[Model] Model name: {self.model_name}')
        print(f'[Model] Classes count: {self.class_num}')
        for c in range(self.class_num):
            cb = ClassBlock(input_dim=self.num_ftrs, class_num=1, activ='sigmoid')
            self.__setattr__('class_%d' % c, cb)
            print(f'[Model] Classes {c}: {cb.name()}')
        print()
